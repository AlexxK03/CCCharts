class ScatterChart{
    ...
    })
    {
        ...
        this.dotWidth = (this.width - (this.margin*2) - ((this.data.getRowCount())*this.gap))/this.data.getRowCount() 
    ...
    dot(){
        push()
        for(let x=0; x<this.data.getRowCount();x++){
            let mean = -this.data.rows[x].obj[this.valueT]
            let gap = x * (this.gap + this.dotWidth)
            let palette = ["#0FA3B1","#241571","#F03A47"]
            let colorNum = x % palette.length
            fill(color(palette[colorNum]))            
            noStroke()
            ellipse(gap+this.gap,this.scaler(mean),10,10)
        }
        pop()
    }
